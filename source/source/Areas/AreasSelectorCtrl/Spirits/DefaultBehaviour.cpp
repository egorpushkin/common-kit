...
namespace AreasSelector
{
	...
	mc::result DefaultBehaviour::OnContextMenu(int x, int y, int globalX, int globalY)
	{
		...
		if ( contextPoint_ )
		{
			...
			menu.addSeparator();
			menu.addAction(editAreaNameAct_);
	...
	// IAgent section
	mc::result DefaultBehaviour::Invoke(mc::DispId idMember, mc::DispParamsRef dispParams, mc::IVariantWrapperRef result, mc::RefIid /* iid */)
	{
		if ( idMember == AGENTID_PROJECTCHANGED )
		{
			ResetState();
		}
		else if ( idMember == AGENTID_DISPLAYPROPERTIES )
		{
			emit showPropertiesSgn();
		}
	...
	void DefaultBehaviour::CreateActions()
	{
		...
		saveProjectAct_ = new QAction(tr("Save Project As..."), this);
		...
		// Point actions
		editAreaNameAct_ = new QAction(tr("Edit Area Name..."), this);
		editAreaNameAct_->setStatusTip(tr("Display control to edit area name"));
		connect(editAreaNameAct_, SIGNAL(triggered()), this, SLOT(editAreaName()));
		...
		connect(this, SIGNAL(showPropertiesSgn()), this, SLOT(showProperties()));
	}
	...
	void DefaultBehaviour::newArea()
	{
		...
		// Ask user for area name
		AreaName areaName(tr("default"), this);
		areaName.exec();
		mc::StringA name(areaName.GetName().toAscii().data());
	...
	void DefaultBehaviour::saveProject()
	{
		...
		ILoaderPtr loader(mc::ObjectCreator< Serialization::AsciiLoader >(), IID_ILoader);
		loader->SaveProject(fileName.toAscii().data(), project);
	}
	...
	void DefaultBehaviour::showProperties()
	{
		...
		// Invalidate control here
	}

	void DefaultBehaviour::editAreaName()
	{
		if ( !contextPoint_ )
			return;
		...
		// Query area from point's parent item
		Areas::IAreaPtr area(pointItem->GetParent(), Areas::IID_IArea);
		if ( !area )
			return;

		AreaName areaName(area->GetName().c_str(), this);
		if ( QDialog::Accepted == areaName.exec() )
		{
			// Update area name
			area->SetName(areaName.GetName().toAscii().data());
		}
	}
	...
	void DefaultBehaviour::newPoint()
	{
		if ( contextPoint_ )
		{
			...
			pointsContainer->InsertAfter(cmnPoint, mc::ICommonPtr(contextPoint_, mc::IID_ICommon));	
	...
	void DefaultBehaviour::deletePoint()
	{
		if ( contextPoint_ )
		{
			...
			else
			{
				...
				// Delete point 
				pointsContainer->Delete(mc::ICommonPtr(contextPoint_, mc::IID_ICommon));		
