...
SelectorProperties::SelectorProperties(QWidget *parent, AreasSelector::PropertiesRef properties_)
	: QDialog(parent)
	, ui()
...
void SelectorProperties::on_labelSize__textChanged(const QString& text)
{
	if ( text.toInt() <= 0 )
	{
		ui.labelSize_->setText("1");
		properties_->labelFont_.setPointSize(1);
	}
	else
