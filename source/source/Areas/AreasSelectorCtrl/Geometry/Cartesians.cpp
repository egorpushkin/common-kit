...
namespace Geometry
{
	...
	int Cartesians::GetXOffset() const
	{
		return xOffset_;
	}

	int Cartesians::GetYOffset() const
	{
		return yOffset_;
	}

	int Cartesians::GetHorizMax() const
	{
		int max = dataWidth_ * scale_ - viewportWidth_;
		return ( max > 0 ) ? ( max ) : ( 0 );
	}

	int Cartesians::GetHorizStep() const
	{
		return dataWidth_;
	}

	int Cartesians::GetVertMax() const
	{
		int max = dataHeight_ * scale_ - viewportHeight_;
		return ( max > 0 ) ? ( max ) : ( 0 );
	}

	int Cartesians::GetVertStep() const
	{
		return dataHeight_;
	}

	float Cartesians::GetScale() const
	{
		return scale_;
	}

	// Configurators
	void Cartesians::ConfigurePainterWindow(int& x, int& y, int& width, int& height) const
	{
		x = xOffset_;
		y = yOffset_;
	...
	void Cartesians::ConfigureSource(int& x, int& y, int& width, int& height) const
	{
		x = xOffset_ / scale_;
		y = yOffset_ / scale_;
	...
	void Cartesians::ConfigureTarget(int& x, int& y, int& width, int& height) const
	{
		x = xOffset_ / scale_;
		y = yOffset_ / scale_;
		width = viewportWidth_ / scale_ + 1;
	...
	int Cartesians::InverseX(int x) const
	{
		return ( x + xOffset_ ) / scale_;
	}

	int Cartesians::InverseY(int y) const
	{
